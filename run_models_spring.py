...
from lib.utils import test_data_spring
...
parser.add_argument('--condition_length', type=int, default=72, help="Time condition on")
...
if __name__ == '__main__':
    ...
    # Training Setup
    log_path = "logs/" + args.alias +"_" + args.dataset +  "_Spr_"  + str(args.condition_length) +  "_Pre_" + str(args.pred_length) + "_" + str(experimentID) + ".log"
    ...
    def train_single_batch(model,batch_dict_encoder,batch_dict_decoder,batch_dict_graph,kl_coef):
        ...
        optimizer.zero_grad()
        train_res = model.compute_all_losses(batch_dict_encoder, batch_dict_decoder, batch_dict_graph,args.num_atoms,edge_lamda = args.edge_lamda, kl_coef=kl_coef,istest=False,need_agg=False)
        # 记录forward中的nfe
    ...
    def train_epoch(epo):
        ...
        for itr in tqdm(range(train_batch)):
            ...
            if itr >= wait_until_kl_inc:
                kl_coef = kl_coef*(1 - 0.99 ** (itr - wait_until_kl_inc))
