...
class GraFormer(nn.Module):
    def __init__(self, adj, hid_dim=128, coords_dim=(2, 3), num_layers=4,
        ...
        for i in range(num_layers):
            _gconv_layers.append(_ResChebGC(adj=None, input_dim=hid_dim, output_dim=hid_dim,
    ...
    def forward(self, x, mask, adj=None):
        out = self.gconv_input(x, adj)
        for i in range(self.n_layers):
            # out = self.atten_layers[i](out, mask) 
            # 去掉，就不会因为LAM卡住，只保留所有ChebGConv
            out = self.gconv_layers[i](out, adj)
        out = self.gconv_output(out, adj)
        return out
