...
class ParseData(object):
    ...
    def load_train_data(self,is_train = True):
        ...
        each_length = self.condition_length+self.pred_length
        self.num_states = features.shape[1]
        self.num_features = features.shape[-1]
        if is_train:
            features = features[:-5, :, :each_length, :]
            features_original = features_original[:-5,:,:each_length,:]
        else:
            features = features[-5:, :, :each_length, :]
            features_original = features_original[-5:,:,:each_length,:]
    ...
    def load_test_data(self,pred_length,condition_length):
        ...
        for i, each_feature in enumerate(features):
            # decoder data
            features_each = each_feature[:,condition_length:condition_length+pred_length,self.args.feature_out_index]  # [N,T2,D]
            tmp = features_original[i]
            features_each_origin = tmp[:, condition_length:condition_length+pred_length, self.args.feature_out_index]  # [N,T2,D]

            graph_each = graphs[i,condition_length:condition_length+pred_length, :, :] # [T2,N,N]
