...
from lstm.load_data_covid import ParseData
...
if __name__=='__main__':
    ...
    def drawline_covid(model, pred_length, condition_length, dataloader,device,args,kl_coef):
        ...
        with torch.no_grad():
            for batch in tqdm(range(num_batch)):
                ...
                resem  = model.encoder(batch_dict_encoder)  # [batch*node, dim]
                pred_node = model.decoder(resem, batch_dict_decoder['data'].shape[1]) # [T2, batch*node,dim]
    ...
    if args.load is not None:
        drawline_covid(model, args.pred_length, args.condition_length, dataloader,
