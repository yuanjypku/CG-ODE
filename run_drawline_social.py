    ...
    def drawline_covid(model, pred_length, condition_length, dataloader,device,args,kl_coef):
        ...
        with torch.no_grad():
            for batch in tqdm(range(num_batch)):
                ...
                for indi in range(len(truth_node)):
                    plt.figure()
                    plt.plot(truth_node[indi].cpu(), 'b')
                    plt.plot(pred_node[indi].cpu(), 'r')
                    plt.legend(['ground_truth_x','ground_truth_y', 'prediction_x',  'prediction_y'])
                    plt.savefig('./figs/batch%d_indi%d.png'%(batch, indi))
                    plt.close()
